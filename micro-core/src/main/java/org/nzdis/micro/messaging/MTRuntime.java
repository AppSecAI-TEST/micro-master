...
import org.nzdis.micro.constants.*;
...
public class MTRuntime extends PlatformConstants {
     ...
     * AWS Identity pool
     */
    public static String aws_identity_pool;
    ...
    private static void shutdownNetwork() {
        if (networkConnector != null) {
            ...
            Enumeration<String> it = propagatedNodes.keys();
            while (it.hasMoreElements()) {
                SocketAddress tempKey = SocketAddress.inflate(it.nextElement());
    ...
    private static void assignProperties(MicroPropertiesMap properties) {
        ...
        if (properties.containsKey(AWS_IDENTITY_POOL_ID)) {
            aws_identity_pool = properties.getString(AWS_IDENTITY_POOL_ID);
        }
        if (properties.containsKey(AWS_REGION)) {
            aws_region = properties.getString(AWS_REGION);
        }
    ...
    private synchronized static void sendMicroFiberBroadcast(MicroMessage message) {
        Enumeration<String> it = registeredMicroFibersMap.keys();
        String sender = message.getSender();
        String tempTarget;
        MicroMessage newMessage = null;
        while (it.hasMoreElements()) {
            tempTarget = it.nextElement();
    ...
    protected static void sendRemotecast(MicroMessage message, String keyPrimitive) {
        ...
        Enumeration<String> it = propagatedNodes.keys();
        while (it.hasMoreElements()) {
            String targetIP = it.nextElement();
     ...
     *
     * @param processName Name of the process by which it is registered in the
    ...
    public static void register(String agentName, MessageCommunicator agent)
        ...
        if (registeredMicroFibersMap.get(agentName) == null && !reservedWords.contains(agentName)) {
            ...
            if (isPropagating && isDistributed) {
                Enumeration<String> it = propagatedNodes.keys();
                while (it.hasMoreElements()) {
                    propagateProcessAdditionToNode(agentName, it.nextElement());
                }
            }
    ...
    public synchronized static void unregister(String agentName) {
        if (registeredMicroFibersMap.containsKey(agentName)) {
            ...
            if (isPropagating && isDistributed) {
                Enumeration<String> it = propagatedNodes.keys();
                while (it.hasMoreElements()) {
                    propagateProcessRemovalToNode(agentName, it.nextElement());
                }
            }
        } else {
            ...
            while (!success) {
                ...
                if (propagatedNodes.size() >= numberOfPlatforms) {
                    ...
                    if (it.hasMoreElements()) {
                        if (MTRuntime.getPropagatedNodes().get(it.nextElement())) {
                            targetNo++;
                        }
                    }
    ...
    public synchronized static void removeAllPropagatedRemoteProcessesOfNode(String node) {
        Enumeration<String> it = propagatedRemoteProcessMap.keys();
        ...
        while (it.hasMoreElements()) {
            ...
            if (propagatedRemoteProcessMap.get(key).contains(node)) {
                ...
                if (tempList.size() == 1) {
                    ...
                    it = propagatedRemoteProcessMap.keys();
                } else {
                    tempList.remove(node);
    ...
    public static void propagateProcessesToNode(SocketAddress address) {
        ...
        while (it.hasMoreElements()) {
            localProcessesToPropagate.put(it.nextElement(), processAdditionKeyword);
        }
